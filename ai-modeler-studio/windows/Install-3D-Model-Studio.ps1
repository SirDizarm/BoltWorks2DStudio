...
Write-Host "Installing 3D Model Studio to $installRoot"
...
if ($DesktopShortcut) {
  New-Shortcut -Path (Join-Path $desktopPath "3D Model Studio.lnk") -TargetPath $shortcutTarget -WorkingDirectory $installRoot -IconLocation $shortcutIcon
}

if ($StartMenuShortcut) {
  New-Shortcut -Path (Join-Path $startMenuDir "3D Model Studio.lnk") -TargetPath $shortcutTarget -WorkingDirectory $installRoot -IconLocation $shortcutIcon
}
...
Set-Item -Path "HKCU:\Software\Classes\$progId" -Value "3D Model Studio Project"
...
Write-Host ""
Write-Host "Installed 3D Model Studio."
